...
select avg(seats_available) as total_seats
from bus_data
where source='Nagpur';

-- 33.	Count distinct sources in the dataset.
select count(distinct source) as distinct_source from bus_data;


-- 34.	Count distinct destinations in the dataset.
select count(distinct destination) as distinct_destination from bus_data;
...
update bus_data set seats_available =50 where source="goa";
